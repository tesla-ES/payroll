...
$result = mysql_query("SELECT import_id,Import_ser,import_rec_dt,import_memo
						FROM import_table
						ORDER BY import_id
						LIMIT $start , $per_page");

$query="select import_id,Import_ser,import_rec_dt,import_memo from import_table where";
if(!empty($Text1)){	$query .=	" (Import_ser like '%$Text1%')  ";}else{	$query .=	"  true  ";}
if(!empty($date1)){
	$query .=	" && (import_rec_dt like '$date1%')  ";
}else{
	$query .=	" && true  ";
}
if(!empty($date2)){
	$query .=	" && (import_send_dt like '$date2%')  ";
}else{
	$query .=	" && true  ";
}
if(!($Text4==0)){
...
$query .="ORDER BY import_id  LIMIT $start , $per_page";
...
$num_rows=1;
	...
	if($j<4){
	  ...
	  echo "<td ALIGN=RIGHT  > <FONT color='#FFFFFF'><a href='list.php?id={$rows[0]}'> $rows[$j]</td>";
...
$next=$start + $per_page;
  ...
  echo "</table>";
    ...
    if($num_rows>$per_page){ 	
		...
		///////////getting the last page
			echo " <a href='list2.php?start=$last_rec'><img src='img/lastpage.gif' alt='Last Page'></a>";
		/////////////getting the next page
			echo " <a href='list2.php?start=$next'><img src='img/nextpage.gif' alt='Next Page'></a>";
 ...
 //////////////////getting previous page
		echo " <a href='list2.php?start=$prev'><img src='img/backpage.gif' alt='Previous Page'></a>";
/////////////getting the first page
		echo " <a href='list2.php?start=0'><img src='img/first.gif' alt='First Page'></a>";
	}
