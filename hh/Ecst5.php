...
///////////////////////////////////////update function
$fin=mysql_query("UPDATE export_table SET export_ser = '$ser_item',export_rec_dt='$rec_dt_item',
export_kind = '$kind_item',export_sender = '$sender_item',
export_send_place = '$send_place_item',
export_memo = '$memo_item',export_to = '$to_item',
export_notes= '$notes_item'
where export_id='$id_item' ")or die(mysql_error());
...
if (file_exists("upload/" . $_FILES["file"]["name"]))
      {
       		unlink("upload/ximg".$id_item .$type);
    		rename ( "upload/" . $_FILES["file"]["name"],"upload/ximg".$id_item .$type);
      }
    
   

   $second_column=  "ximg".$id_item .$type;

 $fin1=mysql_query("select * from export_file where export_id='$id_item' ")or die(mysql_error());
 $n_res1=mysql_num_rows($fin1);

 if($n_res1==0){
   $query1 = "insert into export_file(export_id,file_name) values ('$id_item','$second_column')";

	mysql_query($query1)or  	die (mysql_error());
	}else{
	$query1 = "UPDATE export_file SET  export_id = '$id_item',file_name = '$second_column' where export_id='$id_item'";
...
mysql_close( $link);
	...
	header("Location: http://localhost/Ecst2.php?start=0");
